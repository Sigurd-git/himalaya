...
def solve_ridge_svd(
    ...
    for start in range(0, n_targets, n_targets_batch):
        ...
        nan_targets = backend.isnan(Y[:, batch]).any(0)
        if Y.shape[0] < Y.shape[1]:
            weights_batch_not_nan = (
                (Vt.T @ iUT[~nan_targets]) @ Y.T[~nan_targets, :, None]
            )[:, :, 0].T
        else:
            weights_batch_not_nan = (
                Vt.T @ (iUT[~nan_targets] @ Y.T[~nan_targets, :, None])[:, :, 0].T
            )
        if nan_targets.any():
            nan_samples = backend.isnan(Y[:, nan_targets]).any(1)
            U, eigenvalues, Vt = backend.svd(X[~nan_samples], full_matrices=False)
            inverse = eigenvalues[:, None] / (alpha[None] + eigenvalues[:, None] ** 2)
            iUT = _batch_or_skip(inverse, batch, 1)[:, None, :] * U.T[:, :, None]
            iUT = backend.transpose(iUT, (2, 0, 1))
            if Y.shape[0] < Y.shape[1]:
                weights_batch_nan = (
                    (Vt.T @ iUT[nan_targets]) @ Y.T[nan_targets][:, ~nan_samples, None]
                )[:, :, 0].T
            else:
                weights_batch_nan = (
                    Vt.T
                    @ (iUT[nan_targets] @ Y.T[nan_targets][:, ~nan_samples, None])[
                        :, :, 0
                    ].T
                )

        weights[:, ~nan_targets] = backend.to_cpu(weights_batch_not_nan)
        weights[:, nan_targets] = backend.to_cpu(weights_batch_nan)
    if fit_intercept:
